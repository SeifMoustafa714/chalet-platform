...
export function ListingDetailClient({ id }: { id: string }) {
  const { data: listing, mutate } = useSWR<Listing>(`/listings/${id}`, fetcher);
  ...
  async function submitReview(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.post(`/listings/${id}/reviews`, { rating, comment: comment || undefined });
      setComment('');
      mutate();
    } catch (err: any) {
  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
      ...
      <div className="lg:col-span-1">
        <div className="sticky top-24 space-y-4 rounded-xl border border-ink/10 bg-white p-5">
          <PriceRange pricing={listing.pricing} />
          <div className="flex flex-col gap-2">
            <a href={requestToBookHref} className="btn-primary text-center">Request to book</a>
            <a href={whatsappUrl} className="btn-accent text-center">Ask on WhatsApp</a>
          </div>
          <p className="text-xs text-ink/50">
            Sending a request doesn't charge you anything — our team confirms availability and final price first.
          </p>
        </div>
      </div>
    </div>
